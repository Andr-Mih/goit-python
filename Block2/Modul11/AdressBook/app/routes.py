...
from app import app
from app.form import RecordForm, ContactForm
...
class AddressBook(UserDict):
    ...
    def __str__(self):
        ...
        for key, data in self.data.items():
            result += f'id - {key} | name - {data["name"]} | phones - {data["phones"]} | email - {data["email"]}\n'
        return result
...
def contact1():
    ...
    if form.validate_on_submit():
        name = Name(form.username.data)
        phones = Phone(form.phone.data)
        email = Email(form.email.data)
        address = Address(form.address.data)
        birthday = Birthday(form.birthday.data)
        record = Record(name, phones, email, address, birthday)
        if not load_addressBook():
            ab = AddressBook()
        else:
            ab = load_addressBook()
        ...
        print(f'Data received. Now redirecting')
        return render_template('answer.html', title = 'DATA', data=ab)
        flash("Message Received", "success")
